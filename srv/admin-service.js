...
module.exports = class AdminService extends cds.ApplicationService {
  init() {
    ...
    this.on("uploadAuthorAvatar", async (req) => {
      const { file, authorId } = req.data;

      if (!file || !authorId) {
        return req.error("Missing author ID or file");
      }

      const book = await SELECT.from("Authors").where({ ID: authorId });

      if (book.avatarID) {
        await cloudinary.uploader.destroy(book.avatarID);
      }
